...
swarm* swarm_construct(int particle_number, Map * map){
    ...
    for(int i =0; i<particle_number; i++){
        ...
        s->particle_arr[i]->pos_x = rand()%(map->heigth);
        s->particle_arr[i]->vel_x = 0;
        s->particle_arr[i]->vel_y = 0;
        s->particle_arr[i]->lowest_found_x = 0;
        s->particle_arr[i]->lowest_found_y = 0;
    }
    return s;
}
...
void update_particles(swarm * s, Map *m){
    ...
    for(int i =0; i<s->size; i++){
        ...
        p->vel_x = 1 * p->vel_x + r1 * 1 * (p->lowest_found_x - p->pos_x) + r2 * 1 * (s->lowest_found_x - p->pos_x);
        ...
        p->vel_y = 1*p->vel_y + r1 * 1 * (p->lowest_found_y - p->pos_y) + r2 * 1 * (s->lowest_found_y - p->pos_y);
        //to samo dla skladowej y
        if(p->pos_y + p->vel_y > m->heigth){
            p->vel_y *= -1;
        }
    }
}
